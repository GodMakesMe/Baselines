...
function groupOutputFiles(files: OutputFiles): GroupedOutputs {
  ...
  Object.entries(files).forEach(([filename, artifact]) => {
    ...
    // text_recovery/ outputs are ignored here — ink detection lives on /ink
...
export default function SurfacePage() {
  ...
  const [activeResultTab, setActiveResultTab] = useState<'visualization' | 'segmentation' | 'other'>('visualization');
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      <header className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 p-6 shadow-xl shadow-slate-200/50 dark:shadow-black/50 backdrop-blur-md">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-700 dark:text-cyan-400">Surface Detection</p>
        ...
        <p className="mt-3 max-w-3xl text-slate-600 dark:text-slate-400">
          Upload one ZIP file or multiple TIFF files. The backend runs visualization and U-Net / nnU-Net segmentation as a long-running background job.
          Ink detection runs separately — see the Ink Detection tab.
        </p>
      </header>
